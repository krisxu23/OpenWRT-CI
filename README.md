...
原厂 512MB 内存设备切勿刷入，否则将无法启动甚至变砖。
内存配置已针对 1GB 深度优化（IPQ_MEM_PROFILE_1024 + ATH11K_MEM_PROFILE_1G），非 1GB 设备请勿使用。

🔥 核心亮点
⚡ 全速 NSS 硬件加速
有线 + 无线均启用 NSS 卸载（kmod-ath11k-nss），CPU 占用降低，吞吐量更高。

🐳 Docker 容器原生支持
内核集成必要模块，预装 dockerd 及 luci-app-dockerman，轻松运行容器。

🌐 完整网络功能
nftables 防火墙、WireGuard、Tailscale、SQM QoS、多线聚合、4G/5G 模组驱动。

🧩 丰富 LuCI 应用
代理（homeproxy/nikki/openclash）、存储（diskman/samba4）、DNS（mosdns）、主题 argon、网络测速等。

🤖 全自动编译流水线
GitHub Actions 每日检查源码更新，一键触发，ccache 加速编译。

📦 快速开始
Fork 本仓库 → 进入 Actions 标签页

选择 QCA-ALL 工作流 → 点击 Run workflow

（可选）在输入框中添加额外插件，每行一个 CONFIG_PACKAGE_xxx=y

编译完成后，Releases 页面下载对应固件
...
增减插件：手动触发工作流时，通过 手动调整插件包 输入框添加或移除

测试配置：勾选 仅输出配置文件，可生成 .config 而不编译固件
